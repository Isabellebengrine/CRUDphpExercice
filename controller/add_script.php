<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if(count($formError) == 0) {
            ...
            //si image dans input file : NB $_FILES['picture']['name'] existe no matter what mais sera '' si pas d'img :
            if ($_FILES['picture']['name'] != ""){
                ...
                */
                ...
                // Allow certain file formats and stop php or other type files
                ...
                // if everything is ok, try to upload file
                    ...
                    if (move_uploaded_file($_FILES["picture"]["tmp_name"], $target_file)) {
                        $_SESSION['message'] = "The file ". htmlspecialchars( basename( $_FILES["picture"]["name"])). " has been uploaded.";
                    } else {
            ...
            //écrire req
            $sql = 'INSERT INTO `disc` (`disc_title`, `artist_id`, `disc_label`, `disc_year`, `disc_genre`, `disc_price`, `disc_picture`) VALUES (:title, :artistid,  :label, :annee, :genre, :price, :picture)';//sql ok dans phpmyadmin
            //on prépare requête
            ...
            $query->bindValue(':price', $price);//enlevé PARAM_INT pour pas tronquer prix si décimales
            $query->bindValue(':picture', $picture, PDO::PARAM_STR);
            //on exécute la requête
            $query->execute();
            //on affiche message de confirmation
            $_SESSION['message'] = "Disque ajouté";
            //on ferme connexion
